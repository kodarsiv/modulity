...
class ProviderGenerator implements GeneratorInterface {
    ...
    public function make(): self
    {
        ...
        $path = config('modulity.module_path').DIRECTORY_SEPARATOR.$this->moduleName.DIRECTORY_SEPARATOR."Providers";

        try {
            $fileGenerator = new FileGenerator();
            $fileGenerator->setType(FileGenerator::TYPE_PROVIDERS);
        ...
        try {
            ...
            $data = [
                'phpTagStart' => '<?php',
                'namespace' => config('modulity.namespace') ."\\".$this->moduleName. '\\Providers',
                'className' => $this->fileName . 'Provider',
                'type' => 'class',
                'extendsClassName' => "ServiceProvider",
                "extendsClassNameSpace" => "Illuminate\Support\ServiceProvider"
            ];
